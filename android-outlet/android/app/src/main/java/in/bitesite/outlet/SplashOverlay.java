...
/**
 * The launch splash: a bone field the brand elements pop into. Counterpart of the same
 * class in the student app, with this app's own artwork.
 *
 * <p>Why this exists at all. Android 12 removed custom launch images — the system splash
 * takes a solid colour and one centre icon masked to a circle, and nothing else, so an
 ...
 * A requestLayout issued during layout is deferred, and the second pass either arrives
 * after the animation has started — so it plays at 1x1 and is invisible — or does not
 * arrive at all. Scale and translation are draw-time properties: they take effect on the
 * next frame with no measure or layout pass, so there is no ordering to lose.
 */
public class SplashOverlay extends FrameLayout {

    /** The artwork's own canvas, from outlet-splash.png. */
    private static final float ART_W = 941f;
    private static final float ART_H = 1672f;
    ...
    // Geometry measured off outlet-splash.png by bounding box, so the composition matches
    // the original exactly. The corner pieces start pushed further into their own corner
    // and settle inward, which reads as them arriving rather than merely growing.
    private static final Element[] ELEMENTS = {
        new Element(R.drawable.splash_logo,  0.5016f, 0.4997f, 0.4846f, 0.30f,  0f,     0f,     60L),
        new Element(R.drawable.splash_tray,  0.2455f, 0.2291f, 0.3698f, 0.55f, -0.18f, -0.18f, 200L),
        new Element(R.drawable.splash_bell,  0.7970f, 0.2344f, 0.2657f, 0.55f,  0.18f, -0.18f, 265L),
        new Element(R.drawable.splash_token, 0.2210f, 0.7961f, 0.2763f, 0.55f, -0.18f,  0.18f, 330L),
        new Element(R.drawable.splash_box,   0.7837f, 0.8017f, 0.3135f, 0.55f,  0.18f,  0.18f, 395L),
    };
     ...
     * by the larger ratio keeps the corner pieces bleeding off the edges on any aspect
    ...
    private void playIntro(int screenW, int screenH) {
        ...
        for (int i = 0; i < ELEMENTS.length; i++) {
            ...
            // Uniform: the element PNGs were cut at the artwork's own scale, so width and
            // height resolve to the same factor.
